...
@section('container')
    ...
    <div class="mb-5">
        <div class="relative flex flex-col justify-between w-full bg-white min-h-[80px] p-6 rounded-lg drop-shadow-md">
            <p class="text-3xl font-semibold mb-2 text-center">Deposit Money to Your account</p>
            ...
            <form id="form" action="/deposit" method="POST">
                ...
                <div class="flex justify-end my-4">
                    <button type="submit" class="button-primary w-full !h-20">
                        <p class="text-center text-2xl font-bold">Deposit</p>
                    </button>
                </div>
            </form>
        </div>
                    ...
                    $.ajax({
                        ...
                        if (response.status) {
                            if(response.data.status == 1){
                                toast.html(`<x-toast :success=true >Successfully deposit: ${formatCurrency(response.data.amount)}</x-toast>`)
                                    .children('div').fadeOut(3000);
                                getBalance()
                            } else {
                                toast.html(`<x-toast :error=true >deposit failed</x-toast>`)
                                    .children('div').fadeOut(3000);
                            }
                        ...
                        btn.html('<p class="text-center text-2xl font-bold">Deposit</p>');
